    ...
    def test_propose(self, moves, use_python=True):
        ...
        for movxi, movx in enumerate(moves):
            ...
            for mxi, mx in enumerate(movs):
                ...
                new_indirect_energy = self._charge_indirect_energy_new(pid, mx)

                self._tmp_energies[_ENERGY.U1_INDIRECT][movxi, mxi] = new_indirect_energy
        ...
        for movxi, movx in enumerate(moves):
            ...
            for mxi, mx in enumerate(movs):
                if np.linalg.norm(self.positions.data[pid, :] - mx) < 10.0 ** -14:
                    pid_prop_energy[mxi] = self.energy * self.energy_unit
                else:
                    pid_prop_energy[mxi] = (
                        self.energy + self._tmp_energies[_ENERGY.U_DIFF][movxi, mxi]
                    ) * self.energy_unit
...
class KMCFMM(_PY_KMCFMM, InjectorExtractor):
    ...
    def __init__(
        ...
        self.md = FMMMPIDecomp(self.fmm, self.max_move, boundary_condition=self._bc, cuda=self.cuda_direct)
    ...
    def propose_with_dats(
        ...
        *Fast electrostatic solvers for kinetic Monte Carlo simulations*.
        ...
        cmove_data = self.md.setup_propose_with_dats(
            site_max_counts, current_sites, prop_positions, prop_masks, prop_energy_diffs, prop_charges
        )
    ...
    def _accept(self, move, compute_energy=True):
        ...
        if self.comm.size > 1:
            ...
            if move is not None:
                assert abs(bcast_energy - new_energy) < 10.0 ** -15
    ...
    def _check_ordering_dats(self):
        # self._ordering_win
        if (
            (not hasattr(self.group, "_kmc_fmm_order"))
            or (self.group._kmc_fmm_order.dtype is not INT64)
            or (self.group._kmc_fmm_order.ncomp != 1)
        ):
        ...
        self._ordering_win = MPI.Win.Create(
            self._ordering_buf.array, disp_unit=self._ordering_buf.array.itemsize, comm=self.comm
        )
        ...
        for movxi, movx in enumerate(moves):
            ...
            for mxi, mx in enumerate(movs):
                if np.linalg.norm(self.positions.data[pid, :] - mx) < 10.0 ** -14:
                    ...
                    pid_prop_energy[mxi] = (
                        self.energy + self._tmp_energies[_ENERGY.U_DIFF][movxi, mxi] * self.energy_unit
                    )
