...
x_pos = [ ind_to_pos(px) for px in range(Ns) if ((ind_to_pos(px) >= A.domain.boundary[0]) and (ind_to_pos(px) < A.domain.boundary[1])) ]
y_pos = [ ind_to_pos(px) for px in range(Ns) if ((ind_to_pos(px) >= A.domain.boundary[2]) and (ind_to_pos(px) < A.domain.boundary[3])) ]
z_pos = [ ind_to_pos(px) for px in range(Ns) if ((ind_to_pos(px) >= A.domain.boundary[4]) and (ind_to_pos(px) < A.domain.boundary[5])) ]
N_local = len(x_pos) * len(y_pos) * len(z_pos)

tmp_s = np.zeros(1, INT64)
tmp_r = np.zeros(1, INT64)
tmp_s[:] = N_local

A.domain.comm.Allreduce(tmp_s, tmp_r)
if tmp_r[0] != N: raise RuntimeError('Bad N_local on one or more ranks')

pi = np.zeros((N_local, 3), REAL)
qi = np.zeros((N_local, 1), REAL)

for pxi, posx in enumerate(product(x_pos, y_pos, z_pos)):
    pi[pxi, :] = posx
    qi[pxi, 0] = (-1.) ** pxi
...
# load data into dats
A.P[:N_local, :] = pi.copy()
# hacky way to remove these particles
A.P[N_local::, :] = E
del pi
